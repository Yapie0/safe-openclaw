...
当策略允许工具调用通过后，Execution Isolation 会自动将 exec 类工具的命令包装为受限子进程执行：

- **macOS**：使用 `sandbox-exec` 配置文件限制文件系统和网络访问
- **Linux**：使用 `unshare` 挂载命名空间，将黑名单路径绑定挂载为 `/dev/null`
- **跨平台**：`timeout` 超时控制 + `ulimit` 资源限制（文件大小）
