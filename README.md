...
| --------------- | ----------------------------------------------- | --------- |
| `enforcement`   | `"block"` 拦截 / `"warn"` 仅告警 / `"off"` 关闭 | `"block"` |
| `defaultAction` | 无规则匹配时的默认行为                          | `"deny"`  |
| `auditLog`      | 是否记录审计日志                                | `true`    |

> **内置白名单：** 默认放行主流 AI API（OpenAI、Anthropic、Google、DeepSeek 等）、开发工具（git、node、python、curl 等）和常用服务（GitHub、npm、Notion、Slack 等），同时硬编码禁止危险命令（sudo、dd、mkfs 等）。Skill Hub 中的热门技能无需额外配置即可直接使用。
>
